...
export default function SlidePanel({ open, onClose, title, children }: SlidePanelProps) {
  ...
  return (
    <Portal>
      ...
      />
      {/* Panel */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-md z-[70] bg-white border-l border-hairline shadow-[-12px_0_32px_rgba(0,0,0,0.12)] transition-transform duration-300 flex flex-col ${open ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`}>
