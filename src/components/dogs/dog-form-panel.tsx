...
interface DogFormPanelProps {
  open: boolean; onClose: () => void; onSaved?: (newDogId?: string) => void; editDogId?: string | null; userId: string; defaultLitterId?: string | null; defaultBreedId?: string | null; defaultFatherId?: string | null; defaultMotherId?: string | null; defaultKennelId?: string | null; defaultKennelName?: string | null; defaultAffixFormat?: string | null
}
...
function GCard({label,name,image,sexColor,onClear,selector,disabled}:{label:string;name?:string;image?:string|null;sexColor?:string;onClear:()=>void;selector:React.ReactNode;disabled?:boolean}){
  ...
  return <div ref={ref} className="relative">
    <div onClick={()=>{if(!disabled)setOpen(!open)}} className={`w-full bg-white/5 border border-white/10 rounded-lg p-3 flex items-center gap-2 cursor-pointer transition ${disabled?'opacity-40 cursor-not-allowed':'hover:border-white/20'} ${open?'border-[#D74709]':''}`}>
      {image&&<div className="w-8 h-8 rounded-full border overflow-hidden flex-shrink-0 bg-white/5" style={{borderColor:sexColor||'rgba(255,255,255,0.1)'}}><img src={image} alt="" className="w-full h-full object-cover"/></div>}
      {name?<div className="min-w-0 flex-1"><p className="text-[9px] text-white/30 uppercase font-semibold">{label}</p><p className="text-sm font-semibold text-white truncate">{name}</p></div>:<div className="flex-1"><p className="text-[9px] text-white/30 uppercase font-semibold">{label}</p><p className="text-xs text-white/40 mt-0.5">Seleccionar</p></div>}
      {sexColor&&!image&&<div className="w-1 h-8 rounded-full flex-shrink-0" style={{backgroundColor:sexColor}}/>}
    </div>
    {open&&!disabled&&<div className="absolute z-[80] top-full mt-1 left-0 right-0 min-w-[240px] bg-gray-800 border border-white/10 rounded-lg shadow-xl max-h-52 overflow-hidden">{selector}{name&&<button onClick={()=>{onClear();setOpen(false)}} className="w-full text-left px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 border-t border-white/5">Quitar</button>}</div>}
  </div>
}
