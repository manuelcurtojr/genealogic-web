            ...
            {photos.length > 1 && (
              <>
                <button onClick={prevMobile}
                  className={`absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-fg hover:text-fg transition ${mobileIdx === 0 ? 'opacity-0 pointer-events-none' : ''}`}>
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button onClick={nextMobile}
                  className={`absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-fg hover:text-fg transition ${mobileIdx >= photos.length - 1 ? 'opacity-0 pointer-events-none' : ''}`}>
        ...
        {photos.length > desktopVisible && (
          <>
            <button onClick={() => setDesktopOffset(o => Math.max(0, o - 1))}
              className={`absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-fg hover:text-fg hover:bg-black/60 transition ${desktopOffset === 0 ? 'opacity-0 pointer-events-none' : ''}`}>
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button onClick={() => setDesktopOffset(o => Math.min(desktopMaxOffset, o + 1))}
              className={`absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-fg hover:text-fg hover:bg-black/60 transition ${desktopOffset >= desktopMaxOffset ? 'opacity-0 pointer-events-none' : ''}`}>
              <ChevronRight className="w-5 h-5" />
            </button>
          </>
        )}
