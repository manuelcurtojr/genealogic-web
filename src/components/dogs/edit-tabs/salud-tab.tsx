    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {counts.map(t => { const Icon = t.icon; return (
          <div key={t.key} className="flex-shrink-0 bg-chip border border-hair rounded-lg px-3 py-2 min-w-[110px]">
            ...
            <p className="text-[11px] text-fg-mute mt-0.5">{t.count} registros</p>
          </div>
        )})}
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        ...
        {VET_TYPES.map(t => <button key={t.key} onClick={() => setFilter(t.key)} className={`px-3 py-1 rounded-full text-xs font-medium transition ${filter === t.key ? 'text-white' : 'bg-chip text-fg-dim hover:bg-chip'}`} style={filter === t.key ? { backgroundColor: t.color } : undefined}>{t.label}</button>)}
      </div>
      ...
      {showForm && (
        <div className="bg-chip border border-hair rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowForm(false)} className="text-fg-mute hover:text-fg"><X className="w-4 h-4" /></button>
          </div>
          <div className="flex gap-1.5 flex-wrap">
            {VET_TYPES.map(t => <button key={t.key} onClick={() => setForm(p => ({ ...p, type: t.key }))} className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition ${form.type === t.key ? 'text-white' : 'border-hair text-fg-mute'}`} style={form.type === t.key ? { backgroundColor: t.color, borderColor: t.color } : undefined}>{t.label}</button>)}
          </div>
          <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Nombre (ej: Rabia, Milbemax...)" className="w-full bg-chip border border-hair rounded-lg px-3 py-2 text-sm text-white placeholder:text-fg-mute focus:border-[#D74709] focus:outline-none" />
          ...
          <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Notas" rows={2} className="w-full bg-chip border border-hair rounded-lg px-3 py-2 text-sm text-white placeholder:text-fg-mute focus:border-[#D74709] focus:outline-none resize-none" />
          {/* Files */}
          <div>
            <p className="text-[11px] font-semibold text-fg-dim uppercase tracking-wider mb-1">Archivos / Fotos</p>
      ...
      {/* Records list */}
      {filtered.length === 0 ? (
        <div className="text-center py-8 text-fg-mute"><Stethoscope className="w-8 h-8 mx-auto mb-2 opacity-30" /><p className="text-sm">Sin registros</p></div>
      ) : filtered.map(r => {
        const type = VET_TYPES.find(t => t.key === r.type) || VET_TYPES[0]; const Icon = type.icon
        return (
          <div key={r.id} className="bg-chip border border-hair rounded-lg p-3 flex items-start gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-fg-mute">{new Date(r.date).toLocaleDateString('es-ES')}</p>
              {r.notes && <p className="text-xs text-fg-mute mt-0.5">{r.notes}</p>}
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition items-center">
              <button onClick={() => togglePublic(r)} className="p-1 text-fg-mute hover:text-fg" title={r.is_public ? 'Ocultar del perfil' : 'Mostrar en perfil'}>
                {r.is_public ? <Eye className="w-3.5 h-3.5 text-green-400" /> : <EyeOff className="w-3.5 h-3.5" />}
              </button>
              <button onClick={() => openEdit(r)} className="p-1 text-fg-mute hover:text-fg"><Pencil className="w-3.5 h-3.5" /></button>
              <button onClick={() => handleDelete(r.id)} className="p-1 text-fg-mute hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        )
      })}
