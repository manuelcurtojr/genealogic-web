  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 overflow-x-auto pb-1">
        {counts.map(t => <div key={t.key} className="flex-shrink-0 bg-surface-card border border-hairline rounded-lg px-3 py-2 min-w-[80px]">
          <span className="text-xs font-bold" style={{ color: t.color }}>{t.label}</span>
          <p className="text-[11px] text-muted">{t.count} titulos</p>
        </div>)}
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        ...
        {AWARD_TYPES.map(t => <button key={t.key} onClick={() => setFilter(t.key)} className={`px-3 py-1 rounded-full text-xs font-medium transition ${filter === t.key ? 'text-white' : 'bg-surface-card text-body hover:bg-surface-card'}`} style={filter === t.key ? { backgroundColor: t.color } : undefined}>{t.label}</button>)}
      </div>

      <button onClick={openAdd} className="flex items-center gap-1.5 text-sm text-ink hover:opacity-80 transition font-medium"><Plus className="w-4 h-4" /> Añadir titulo</button>

      {showForm && (
        <div className="bg-surface-card border border-hairline rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between"><p className="text-sm font-semibold">{editAward ? 'Editar titulo' : 'Nuevo titulo'}</p><button onClick={() => setShowForm(false)} className="text-muted hover:text-ink"><X className="w-4 h-4" /></button></div>
          <div className="flex gap-1.5 flex-wrap">
            {AWARD_TYPES.map(t => <button key={t.key} onClick={() => setForm(p => ({ ...p, award_type: t.key }))} className={`px-2.5 py-1 rounded-full text-[11px] font-bold border transition ${form.award_type === t.key ? 'text-white' : 'border-hairline text-muted'}`} style={form.award_type === t.key ? { backgroundColor: t.color, borderColor: t.color } : undefined}>{t.label}</button>)}
          </div>
          ...
          {/* Files */}
          <div>
            <p className="text-[11px] font-semibold text-body uppercase tracking-wider mb-1">Certificados / Fotos</p>
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-1.5 text-xs text-body">Cancelar</button>
            <button onClick={handleSave} disabled={saving || !form.event_name.trim()} className="bg-ink text-on-primary hover:opacity-90 px-4 py-1.5 rounded-lg text-xs font-semibold transition disabled:opacity-50 flex items-center gap-1">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-8 text-muted"><Trophy className="w-8 h-8 mx-auto mb-2 opacity-30" /><p className="text-sm">Sin titulos</p></div>
      ) : filtered.map(a => {
        ...
        return (
          <div key={a.id} className="bg-surface-card border border-hairline rounded-lg p-3 flex items-start gap-3 group">
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition items-center">
              <button onClick={async () => { await supabase.from('awards').update({ is_public: !a.is_public }).eq('id', a.id); load() }} className="p-1 text-muted hover:text-ink" title={a.is_public ? 'Ocultar' : 'Mostrar'}>
                {a.is_public ? <Eye className="w-3.5 h-3.5 text-green-400" /> : <EyeOff className="w-3.5 h-3.5" />}
              </button>
              <button onClick={() => openEdit(a)} className="p-1 text-muted hover:text-ink"><Pencil className="w-3.5 h-3.5" /></button>
              <button onClick={() => handleDelete(a.id)} className="p-1 text-muted hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        )
      })}
