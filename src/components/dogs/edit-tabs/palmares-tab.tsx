  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 overflow-x-auto pb-1">
        {counts.map(t => <div key={t.key} className="flex-shrink-0 bg-white/5 border border-white/10 rounded-lg px-3 py-2 min-w-[80px]">
      ...
      {showForm && (
        <div className="bg-white/5 border border-white/10 rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between"><p className="text-sm font-semibold">{editAward ? 'Editar titulo' : 'Nuevo titulo'}</p><button onClick={() => setShowForm(false)} className="text-white/30 hover:text-white"><X className="w-4 h-4" /></button></div>
          <div className="flex gap-1.5 flex-wrap">
            {AWARD_TYPES.map(t => <button key={t.key} onClick={() => setForm(p => ({ ...p, award_type: t.key }))} className={`px-2.5 py-1 rounded-full text-[11px] font-bold border transition ${form.award_type === t.key ? 'text-white' : 'border-white/10 text-white/40'}`} style={form.award_type === t.key ? { backgroundColor: t.color, borderColor: t.color } : undefined}>{t.label}</button>)}
          </div>
          <input value={form.event_name} onChange={e => setForm(p => ({ ...p, event_name: e.target.value }))} placeholder="Nombre del evento" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/25 focus:border-[#D74709] focus:outline-none" />
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-8 text-white/30"><Trophy className="w-8 h-8 mx-auto mb-2 opacity-30" /><p className="text-sm">Sin titulos</p></div>
      ) : filtered.map(a => {
