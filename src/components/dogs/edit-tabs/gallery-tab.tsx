...
export default function GalleryTab({ dogId, userId }: GalleryTabProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-[11px] text-muted">Arrastra para reordenar. La primera foto sera la de perfil.</p>

      <div className="flex gap-2 flex-wrap">
        {/* Upload button */}
        <button onClick={() => fileRef.current?.click()} disabled={uploading}
          className="w-[88px] h-[88px] border-2 border-dashed border-white/15 rounded-lg flex flex-col items-center justify-center hover:border-hairline transition cursor-pointer flex-shrink-0">
          {uploading ? <Loader2 className="w-5 h-5 animate-spin text-muted" /> : <Plus className="w-5 h-5 text-muted" />}
        </button>
