    ...
    <div ref={ref} className="relative w-full">
      <div className="relative">
        ...
        <input
          ...
          placeholder={t('Buscar perros, criaderos, razas...')}
      ...
      {open && query.trim().length >= MIN_ANY && (
        <div className="absolute top-full mt-1 left-0 right-0 bg-surface-card border border-hairline rounded-xl shadow-2xl overflow-hidden z-50 max-h-80 overflow-y-auto">
          {results.length === 0 ? (
            searching ? (
              <div className="p-4 text-center text-muted text-sm">{t('Buscando...')}</div>
            ) : (
              <div className="p-4 text-center text-muted text-sm">{t('Sin resultados para')} &ldquo;{query.trim()}&rdquo;</div>
            )
          ) : (
            <>
                ...
                return (
                  <div key={type}>
                    <div className="px-3 py-1.5 text-[10px] text-muted uppercase tracking-wider font-semibold bg-surface-card">
                      {t(type === 'dog' ? 'Perros' : type === 'kennel' ? 'Criaderos' : 'Razas')}
                    </div>
                    {items.map(r => {
              ...
              >
                <Search className="h-3.5 w-3.5" /> {t('Ver todos los resultados de')} &ldquo;{query.trim()}&rdquo;
              </button>
            </>
          )}
        </div>
