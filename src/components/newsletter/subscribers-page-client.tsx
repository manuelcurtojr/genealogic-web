...
export default function SubscribersPageClient({ kennelId, kennelName, initialSubscribers }: Props) {
  ...
  async function handleImport() {
    ...
    if (emails.length === 0) {
      alert('No se detectaron emails válidos')
      return
    }
    if (!confirm(`¿Importar ${emails.length} email${emails.length === 1 ? '' : 's'} a la lista?`)) return
    setImporting(true)
    try {
      ...
      alert(`✓ Importados: ${data.created?.length || 0}\nDuplicados ignorados: ${data.skipped || 0}`)
      setImportText('')
      setImportOpen(false)
    } catch (err: any) {
      alert(err.message)
    ...
    <div>
      ...
      <div className="flex items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">
            {kennelName} · {stats.total} suscriptor{stats.total === 1 ? '' : 'es'} · {stats.active} activo{stats.active === 1 ? '' : 's'}
          </p>
        </div>
      ...
      <div className="rounded-xl border border-hairline bg-surface-card p-4 mb-6 flex items-start gap-3">
        ...
        <div className="text-sm text-body leading-relaxed">
          Aquí gestionas tu lista de suscriptores. El envío de campañas (newsletter, avisos de
          camada) llega en la próxima fase con plantillas y métricas de apertura. Por ahora puedes
          construir y limpiar tu lista — importar emails desde otras herramientas o añadir manualmente.
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        {([
          ...
          { key: 'inactive', label: `Bajas (${stats.inactive})` },
        ...
        <div className="text-center py-16 border border-dashed border-hairline rounded-xl bg-canvas">
          <p className="text-sm text-muted">
            {subscribers.length === 0
              ...
              : 'Ningún suscriptor coincide con el filtro.'}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-hairline">
              <tr className="text-left">
                ...
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted hidden md:table-cell">Nombre</th>
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted hidden lg:table-cell">Origen</th>
      ...
      {importOpen && (
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold text-ink mb-1">Importar suscriptores</h2>
