    ...
    <>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#D74709]/50 transition"
        />
      </div>

      <p className="text-xs text-white/30 mb-3">{filtered.length} criadero{filtered.length !== 1 ? 's' : ''}</p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  {kennel.foundation_date && (
                    <p className="text-xs text-white/30 mt-0.5">Fundado en {new Date(kennel.foundation_date).getFullYear()}</p>
                  )}
                  {kennel.description && (
                    <p className="text-xs text-white/40 mt-1 line-clamp-2">{kennel.description}</p>
                  )}
                </div>
              </div>
            </Link>
