...
export default function AdminGenealogyClient({ dogs, allDogs, breeds, colors, userId }: Props) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 bottom-0 z-20 bg-canvas border-r border-hairline flex flex-col transition-all duration-300"
        style={{ left: sidebarWidth, width: dogPanelPx }}>

        {/* Header — aligned with admin sidebar header */}
        <div className={`h-14 border-b border-hairline flex items-center flex-shrink-0 ${listCollapsed ? 'justify-center' : 'px-2'}`}>
          ...
          {!listCollapsed && <span className="text-xs font-semibold text-muted ml-1">Perros</span>}
        </div>

        {!listCollapsed && (
          <>
            <div className="px-2 pt-2 pb-1 flex-shrink-0">
              <div className="flex bg-surface-card border border-hairline rounded-lg overflow-hidden mb-2">
                {[
                  ...
                  <button key={f.key} onClick={() => setFilter(f.key as any)}
                    className={`flex-1 px-1 py-1.5 text-[9px] font-medium transition ${filter === f.key ? 'bg-surface-card text-ink' : 'text-muted hover:text-body'}`}>
                    {f.label}
                  </button>
                ))}
              </div>
        ...
        {panelMode === 'add' && panelTarget && (
          <>
            <div className="flex items-center justify-between px-5 py-3 border-b border-hairline flex-shrink-0">
              ...
              <button onClick={() => setPanelOpen(false)} className="text-muted hover:text-ink"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex border-b border-hairline flex-shrink-0">
              <button onClick={() => setAncestorMode('existing')}
                className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs font-medium transition border-b-2 -mb-px ${ancestorMode === 'existing' ? 'border-ink text-ink' : 'border-transparent text-muted'}`}>
                <Link2 className="w-3.5 h-3.5" /> Existente
              </button>
              <button onClick={() => setAncestorMode('new')}
                className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs font-medium transition border-b-2 -mb-px ${ancestorMode === 'new' ? 'border-ink text-ink' : 'border-transparent text-muted'}`}>
              ...
              {ancestorMode === 'existing' ? (
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    {ancestorCandidates.length === 0 && <p className="text-xs text-muted text-center py-6">Sin resultados</p>}
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select value={newAncestor.breed_id} onChange={e => setNewAncestor(p => ({ ...p, breed_id: e.target.value }))}
                      className="w-full bg-surface-card border border-hairline rounded-lg px-3 py-2.5 text-sm text-body focus:border-ink focus:outline-none appearance-none">
                  ...
                  <button onClick={createAndLink} disabled={saving || !newAncestor.name.trim()}
                    className="w-full bg-ink text-on-primary hover:opacity-90 px-4 py-2.5 rounded-lg text-sm font-semibold transition disabled:opacity-50 flex items-center justify-center gap-1.5">
