          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
            ...
            {ownerInfo && (
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-white/30 truncate">{ownerInfo.email}</p>
                </div>
                <span className="text-[9px] text-white/20 ml-auto">Propietario</span>
              </div>
            )}
            ...
            <Section title="Datos generales" icon={Store}>
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider mb-1 block">Descripción</label>
                ...
                <div>
                  <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider mb-1 block">Formato de afijo</label>
                  ...
                  return (
                    <button key={b.id} onClick={() => toggleBreed(b.id)}
                      className={`text-[10px] font-medium px-2.5 py-1 rounded-full transition ${
                        selected ? 'bg-[#D74709]/15 text-[#D74709] border border-[#D74709]/30' : 'bg-white/5 text-white/40 border border-white/10 hover:border-white/20'
                      }`}>
                      {b.name}
                    </button>
                  )
            ...
            <Section title="WhatsApp" icon={MessageCircle}>
              ...
              {form.whatsapp_enabled && (
                <div className="space-y-3 pl-6 border-l-2 border-green-500/30">
                  ...
                  <div>
                    <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider mb-1 block">Mensaje predeterminado</label>
                    <textarea value={form.whatsapp_text} onChange={e => set('whatsapp_text', e.target.value)} rows={2}
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 focus:border-[#D74709] focus:outline-none resize-none" />
                  </div>
                </div>
              )}
            </Section>
            ...
            <div className="border-t border-red-500/20 pt-4">
              {showDeleteConfirm ? (
                <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4 space-y-3">
                  ...
                  <p className="text-xs text-white/30">Se desvincularán todos los perros del criadero y se eliminarán los formularios y solicitudes.</p>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setShowDeleteConfirm(false)} className="text-white/50 hover:text-white px-4 py-2 text-sm">Cancelar</button>
                  </div>
                </div>
              ) : (
