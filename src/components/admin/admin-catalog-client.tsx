  ...
  return (
    <div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden divide-y divide-white/5">
        {filtered.map(item => (
          <div key={item.id}
            className={`flex items-center gap-3 px-4 py-3 hover:bg-white/[0.02] transition cursor-pointer ${
            ...
            {tab === 'colors' && (
              <div className="w-8 h-8 rounded-lg overflow-hidden bg-white/5 border border-white/10 flex-shrink-0">
                {item.thumbnail_url ? <img src={item.thumbnail_url} alt="" className="w-full h-full object-cover" /> :
                  <div className="w-full h-full flex items-center justify-center"><Palette className="w-3 h-3 text-white/15" /></div>}
              </div>
            )}
            {editingId === item.id ? (
              <>
                ...
                <button onClick={e => { e.stopPropagation(); setEditingId(null) }} className="text-white/30 hover:text-white/60"><X className="w-4 h-4" /></button>
              </>
            ) : (
              <>
                ...
                {tab === 'breeds' && (
                  <>
                    <span className="text-[10px] text-white/30">{item.dog_count} perros</span>
                ...
                <button onClick={e => { e.stopPropagation(); setEditingId(item.id); setEditName(item.name) }} className="text-white/20 hover:text-white/60 transition"><Pencil className="w-3.5 h-3.5" /></button>
                <button onClick={e => { e.stopPropagation(); deleteItem(item.id) }} className="text-white/20 hover:text-red-400 transition"><Trash2 className="w-3.5 h-3.5" /></button>
              </>
            )}
          </div>
        ))}
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md z-[70] bg-gray-900 border-l border-white/10 shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 flex-shrink-0">
              <div>
                ...
                <p className="text-[10px] text-white/30">{breedLinkedColors.length} colores seleccionados</p>
              </div>
              <button onClick={() => setBreedPanel(null)} className="text-white/30 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <p className="px-5 py-2 text-xs text-white/40 bg-white/[0.02] border-b border-white/5">Selecciona los colores que son estándar para esta raza. Solo estos aparecerán al añadir un perro de esta raza.</p>
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              {colors.map(c => {
                const isLinked = breedLinkedColors.includes(c.id)
                return (
                  <button key={c.id} onClick={() => toggleBreedColor(breedPanel, c.id)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition text-left ${isLinked ? 'bg-[#D74709]/10 border border-[#D74709]/30' : 'bg-white/[0.02] border border-transparent hover:bg-white/5'}`}>
                    <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${isLinked ? 'bg-[#D74709] border-[#D74709]' : 'border border-white/20 bg-white/5'}`}>
                      {isLinked && <Check className="w-3 h-3 text-white" />}
                    </div>
                    <div className="w-7 h-7 rounded-md overflow-hidden bg-white/5 border border-white/10 flex-shrink-0">
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md z-[70] bg-gray-900 border-l border-white/10 shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 flex-shrink-0">
              ...
              <button onClick={() => setColorPanel(null)} className="text-white/30 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-white/5 border border-white/10 flex-shrink-0">
                  {colorForm.thumbnail_url ? <img src={colorForm.thumbnail_url} alt="" className="w-full h-full object-cover" /> :
                    <div className="w-full h-full flex items-center justify-center"><Image className="w-6 h-6 text-white/15" /></div>}
                </div>
                <div className="flex-1">
                  <p className="font-bold">{colorForm.name || 'Sin nombre'}</p>
                  <p className="text-[10px] text-white/30">{colorLinkedBreeds.length} razas asignadas</p>
                </div>
              </div>

              {/* Form */}
              <div>
                <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider mb-1 block">Nombre del color *</label>
                <input type="text" value={colorForm.name} onChange={e => setColorForm(p => ({ ...p, name: e.target.value }))}
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#D74709] focus:outline-none" />
              </div>
              <div>
                <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider mb-1 block">URL de la foto del color</label>
                <input type="text" value={colorForm.thumbnail_url} onChange={e => setColorForm(p => ({ ...p, thumbnail_url: e.target.value }))}
                  placeholder="https://..."
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-white/20 focus:border-[#D74709] focus:outline-none" />
                <p className="text-[10px] text-white/20 mt-1">Esta foto aparecerá en el selector de colores al añadir un perro</p>
              </div>
              ...
              <div className="border-t border-white/10 pt-4">
                <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-3">Razas con este color</h3>
                {colorLinkedBreeds.length === 0 ? (
                  <p className="text-xs text-white/20 text-center py-4">Este color no está asignado a ninguna raza</p>
                ) : (
                  <div className="space-y-1">
                    {colorLinkedBreeds.map(breedId => {
                      ...
                      return (
                        <div key={breedId} className="flex items-center gap-2 px-3 py-2 bg-white/[0.02] rounded-lg">
                          <Dog className="w-3.5 h-3.5 text-white/30" />
                          <span className="text-xs font-medium flex-1">{breed.name}</span>
                          <span className="text-[10px] text-white/20">{breed.dog_count} perros</span>
                        </div>
                      )
                    })}
                  </div>
