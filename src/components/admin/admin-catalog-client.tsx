...
export default function AdminCatalogClient({ breeds: initBreeds, colors: initColors, breedColors: initBC }: Props) {
  ...
  return (
    <div>
      ...
      <p className="text-muted text-sm mb-6">Gestión de razas y colores</p>

      {/* Tabs */}
      <div className="flex gap-2 mb-4">
        <button onClick={() => { setTab('breeds'); setSearch(''); setNewName(''); setBreedPanel(null); setColorPanel(null) }}
          className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === 'breeds' ? 'bg-surface-card text-ink' : 'bg-surface-card text-body hover:bg-surface-card'}`}>
        ...
        <button onClick={() => { setTab('colors'); setSearch(''); setNewName(''); setBreedPanel(null); setColorPanel(null) }}
          className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === 'colors' ? 'bg-surface-card text-ink' : 'bg-surface-card text-body hover:bg-surface-card'}`}>
      ...
      <div className="bg-surface-card border border-hairline rounded-xl overflow-hidden divide-y divide-white/5">
        {filtered.map(item => (
          <div key={item.id}
            className={`flex items-center gap-3 px-4 py-3 hover:bg-surface-card transition cursor-pointer ${
              (tab === 'breeds' && breedPanel === item.id) || (tab === 'colors' && colorPanel === item.id) ? 'bg-surface-soft' : ''
            }`}
            onClick={() => {
            ...
            {editingId === item.id ? (
              ...
              <>
                ...
                {tab === 'breeds' && (
                  <>
                    <span className="text-[10px] text-muted">{item.dog_count} perros</span>
                ...
                <button onClick={e => { e.stopPropagation(); setEditingId(item.id); setEditName(item.name) }} className="text-muted hover:text-body transition"><Pencil className="w-3.5 h-3.5" /></button>
                <button onClick={e => { e.stopPropagation(); deleteItem(item.id) }} className="text-muted hover:text-red-400 transition"><Trash2 className="w-3.5 h-3.5" /></button>
              </>
            )}
          </div>
        ))}
      ...
      {breedPanel && selectedBreed && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md z-[70] bg-surface-card border-l border-hairline shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-hairline flex-shrink-0">
              <div>
                ...
                <p className="text-[10px] text-muted">{breedLinkedColors.length} colores seleccionados</p>
              </div>
              <button onClick={() => setBreedPanel(null)} className="text-muted hover:text-ink"><X className="w-5 h-5" /></button>
            </div>
            <p className="px-5 py-2 text-xs text-muted bg-surface-card border-b border-hairline">Selecciona los colores que son estándar para esta raza. Solo estos aparecerán al añadir un perro de esta raza.</p>
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              {colors.map(c => {
                const isLinked = breedLinkedColors.includes(c.id)
                return (
                  <button key={c.id} onClick={() => toggleBreedColor(breedPanel, c.id)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition text-left ${isLinked ? 'bg-surface-card border border-hairline' : 'bg-surface-card border border-transparent hover:bg-surface-card'}`}>
                    <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${isLinked ? 'bg-ink border-ink' : 'border border-hairline bg-surface-card'}`}>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md z-[70] bg-surface-card border-l border-hairline shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-hairline flex-shrink-0">
              ...
              <button onClick={() => setColorPanel(null)} className="text-muted hover:text-ink"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-muted">{colorLinkedBreeds.length} razas asignadas</p>
                </div>
              </div>
              ...
              <button onClick={saveColor} disabled={saving || !colorForm.name.trim()}
                className="w-full bg-ink text-on-primary hover:opacity-90 font-semibold px-4 py-2.5 rounded-lg transition disabled:opacity-50 flex items-center justify-center gap-1.5 text-sm">
              ...
              <div className="border-t border-hairline pt-4">
                <h3 className="text-xs font-semibold text-muted uppercase tracking-wider mb-3">Razas con este color</h3>
                {colorLinkedBreeds.length === 0 ? (
                  <p className="text-xs text-muted text-center py-4">Este color no está asignado a ninguna raza</p>
                ) : (
                  <div className="space-y-1">
                    {colorLinkedBreeds.map(breedId => {
                      ...
                      return (
                        <div key={breedId} className="flex items-center gap-2 px-3 py-2 bg-surface-card rounded-lg">
                          <Dog className="w-3.5 h-3.5 text-muted" />
                          <span className="text-xs font-medium flex-1">{breed.name}</span>
                          <span className="text-[10px] text-muted">{breed.dog_count} perros</span>
                        </div>
                      )
                    })}
                  </div>
