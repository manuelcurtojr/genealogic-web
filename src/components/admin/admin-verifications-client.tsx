    ...
    <div className="p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-white/40">Revisar solicitudes de verificacion de perros</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {items.map(v => {
            ...
            return (
              <div key={v.id} className={`border rounded-xl p-4 ${v.status === 'approved' ? 'border-green-500/20 bg-green-500/5' : v.status === 'rejected' ? 'border-red-500/20 bg-red-500/5' : 'border-white/10'}`}>
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-xl overflow-hidden bg-white/5 flex-shrink-0">
                    {v.dog_photo ? <img src={v.dog_photo} alt="" className="w-full h-full object-cover" /> : <Shield className="w-5 h-5 text-white/10 m-auto" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/30 mt-0.5">{v.owner_name && <span>{v.owner_name} · </span>}{formatted}</p>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      ) : <span className="text-[10px] text-white/20">Sin microchip</span>}
                      {v.pedigree_url ? (
                      ...
                      ) : <span className="text-[10px] text-white/20">Sin pedigri</span>}
                    </div>
                    ...
                    {v.status === 'pending' && (
                      <div className="flex items-end gap-2 mt-3">
                        <input value={adminNotes[v.id] || ''} onChange={e => setAdminNotes(prev => ({ ...prev, [v.id]: e.target.value }))}
                          placeholder="Notas (opcional)..." className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder:text-white/20 focus:border-[#D74709] focus:outline-none" />
