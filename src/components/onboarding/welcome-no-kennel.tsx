...
export default async function WelcomeNoKennel({
  displayName,
  isClient,
}: {
    ...
    <div className="max-w-3xl mx-auto py-8">
      ...
      {isClient && (
        <div className="rounded-2xl border border-hairline bg-canvas p-5 mb-6">
          <p className="text-sm font-semibold text-ink mb-1">{t('¿Vienes a ver el perro que reservaste?')}</p>
          <p className="text-xs text-body mb-4">
            {t('Tu panel de propietario ya tiene tus reservas y perros recibidos.')}
          </p>
          <Link
            href="/mis-reservas"
      ...
      <div className="rounded-2xl border-2 border-ink bg-canvas p-6 sm:p-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-xl font-bold text-ink">{t('Empieza creando tu criadero')}</h2>
        </div>
        <p className="text-sm text-body mb-5">
          {t('Registra tu afijo, tu raza principal y tus datos de contacto. Después podrás añadir perros, configurar tu web pública y empezar a recibir consultas de clientes.')}
        </p>

        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <BenefitItem icon={Dog} title={t('Genealogía completa')}>
            {t('Registra perros con padres, abuelos y descendencia. Búsqueda y exportación incluidos.')}
          </BenefitItem>
          <BenefitItem icon={Globe} title={t('Web pública')}>
            {t('Tu propio sitio en')} <code>genealogic.io/c/tu-afijo</code> {t('o tu dominio personalizado.')}
          </BenefitItem>
          <BenefitItem icon={Mail} title={t('Gestión de clientes')}>
            {t('Pipeline de reservas, contratos, mensajería bidireccional con cada interesado.')}
          </BenefitItem>
          <BenefitItem icon={BarChart3} title={t('Analíticas reales')}>
            {t('Quién visita tu web, de dónde, qué perros miran más. Sin Google Analytics ni cookies.')}
          </BenefitItem>
        </ul>
        ...
        <p className="text-[11px] text-muted text-center mt-3">
          {t('Tarda menos de 3 minutos. Después puedes ir editando todo a tu ritmo.')}
        </p>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-6 text-xs text-muted">
        <Link href="/kennels" className="hover:text-ink">
          {t('Explorar otros criaderos →')}
        </Link>
        <Link href="/search" className="hover:text-ink">
          {t('Buscar perros por raza →')}
        </Link>
