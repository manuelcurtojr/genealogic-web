...
export default function StatCard({ icon: Icon, label, value, accentColor }: StatCardProps) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-2.5 sm:p-3 flex items-center gap-2 sm:gap-3" style={{ borderLeftWidth: 3, borderLeftColor: accentColor }}>
      ...
      <div className="min-w-0">
        ...
        <p className="text-[9px] sm:text-[10px] text-white/40 truncate">{label}</p>
      </div>
    </div>
  )
}
