    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <Link href="/litters" className="text-fg-mute hover:text-fg transition"><ArrowLeft className="w-5 h-5" /></Link>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold">{father?.name || '?'} × {mother?.name || '?'}</h1>
              {breedName && <span className="text-xs bg-chip text-fg-dim rounded-full px-2 py-0.5">{breedName}</span>}
            </div>
            {kennel && (
              <Link href={`/kennels/${kennel.id}`} className="text-xs text-fg-mute hover:text-[#D74709] transition flex items-center gap-1 mt-0.5">
      ...
      <div className="flex gap-4 mb-6">
        ...
        {/* Visual timeline */}
        <div className="flex-1 bg-chip border border-hair rounded-xl p-4 flex items-center">
          <div className="flex items-center w-full">
            {steps.map((step, i) => (
              <div key={step.key} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step.active ? 'bg-[#D74709] text-white' : 'bg-chip text-fg-mute'}`}>
                    {step.active ? <Check className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                  </div>
                  <p className={`text-[10px] font-semibold mt-1 ${step.active ? 'text-[#D74709]' : 'text-fg-mute'}`}>{step.label}</p>
                  {step.date && step.active && (
                    <p className="text-[9px] text-fg-mute">{new Date(step.date).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: '2-digit' })}</p>
                  )}
                </div>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-2 ${steps[i + 1].active ? 'bg-[#D74709]' : 'bg-chip'}`} />
                )}
              </div>
            ))}
          </div>
          {/* Extra info */}
          <div className="flex items-center gap-3 ml-4 pl-4 border-l border-hair flex-shrink-0">
            {litter.puppy_count != null && (
              <div className="text-center">
                <p className="text-lg font-bold">{litter.puppy_count}</p>
                <p className="text-[9px] text-fg-mute">cachorros</p>
              </div>
            )}
            {isOwner && (
              <div className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${litter.is_public ? 'bg-green-500/10 text-green-400' : 'bg-chip text-fg-mute'}`}>
      ...
      {puppies.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-fg-mute uppercase tracking-wider mb-2 flex items-center gap-1.5">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {puppies.map((pup: any) => {
              ...
              return (
                <Link key={pup.id} href={`/dogs/${pup.slug || pup.id}`} className="flex items-center gap-2 bg-chip border border-hair rounded-lg px-3 py-2 hover:border-[#D74709]/30 transition flex-shrink-0">
                  <div className="w-8 h-8 rounded-full border-2 overflow-hidden bg-chip flex-shrink-0" style={{ borderColor: sc }}>
                    {pup.thumbnail_url ? <img src={pup.thumbnail_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-fg-mute text-xs">{pup.sex === 'male' ? '♂' : '♀'}</div>}
                  </div>
      ...
      {pedigreeData.length > 1 && pedigreeRootId && (
        <div className="-mx-4 lg:mx-0 mb-6">
          <h2 className="text-sm font-semibold text-fg-mute uppercase tracking-wider mb-3 px-4 lg:px-0">Genealogia</h2>
...
function CompactParent({ parent, role }: { parent: any; role: string }) {
  ...
  if (!parent) return <div className="w-24 h-24 bg-chip border border-hair rounded-xl flex items-center justify-center text-fg-mute text-sm">{role} ?</div>
  return (
    <Link href={`/dogs/${parent.slug || parent.id}`} className="relative w-24 h-24 rounded-xl overflow-hidden border-2 hover:border-[#D74709] transition group" style={{ borderColor: sc }}>
      {parent.thumbnail_url ? <img src={parent.thumbnail_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-chip flex items-center justify-center text-2xl" style={{ color: sc + '40' }}>{isFather ? '♂' : '♀'}</div>}
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent px-2 pb-1.5 pt-4">
        <p className="text-[10px] text-fg-dim">{role}</p>
