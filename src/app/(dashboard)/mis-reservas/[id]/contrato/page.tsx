...
export default async function MyReservationContractPage({
  ...
  return (
    <div>
      ...
      <h1 className="text-3xl font-bold tracking-tight text-ink mb-1">Contrato</h1>
      <p className="text-sm text-body mb-6">
        Reserva con <strong>{reservation.kennel?.name}</strong>
      </p>

      {!visibleToClient ? (
        <WaitingForBreeder kennelName={reservation.kennel?.name} />
      ) : (
...
function WaitingForBreeder({ kennelName }: { kennelName?: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-hairline bg-canvas p-10 text-center">
      ...
      <p className="text-base font-semibold text-ink">El contrato aún no está disponible</p>
      <p className="mt-2 text-sm text-muted max-w-md mx-auto">
        {kennelName ? <><strong>{kennelName}</strong> está</> : 'El criador está'} preparando
        el contrato. Te avisaremos cuando esté listo para revisar y firmar.
      </p>
    </div>
  )
}
...
function StatusPanel({ contract }: { contract: any }) {
  ...
  return (
    <section className="rounded-2xl border border-hairline bg-canvas p-5">
      <h3 className="text-xs font-bold uppercase tracking-wider text-ink mb-3">Estado</h3>
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      <div className="min-w-0">
        ...
        {done ? (
          <>
            <p className="font-semibold text-ink truncate">{name || 'Firmado'}</p>
