...
import { Dog, Baby, Users, HandCoins, PawPrint, Tag, TrendingUp, FileText, Plus, Calendar, Stethoscope, ArrowRight } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  // Get kennel
  const { data: kennelArr } = await supabase.from('kennels').select('id').eq('owner_id', user.id).limit(1)
  const kennelId = kennelArr?.[0]?.id
  ...
  const [
    ...
    // Recent submissions
    kennelId ? supabase.from('form_submissions').select('id, data, created_at').eq('kennel_id', kennelId).order('created_at', { ascending: false }).limit(5) : Promise.resolve({ data: [] }),
    // Recent deals
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold">Hola, {profile?.display_name || 'Criador'}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold">Proximos eventos</h2>
              ...
              {(activeLittersRes.data || []).map((litter: any) => (
                <Link key={litter.id} href={`/litters/${litter.id}`} className="flex items-center gap-3 bg-purple-500/5 rounded-lg p-2.5 hover:bg-purple-500/10 transition">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-purple-400">{litter.status === 'mated' ? 'En gestacion' : 'Planificada'}</p>
                  </div>
                </Link>
              ))}
      ...
      {/* Row 4: Submissions + Notifications */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        {/* Recent submissions */}
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
