...
export default async function KennelDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch all kennel dogs (visible by default unless user explicitly hides them)
    ...
    <div>
      ...
      <div className="relative bg-gradient-to-r from-[#D74709]/20 to-[#D74709]/5 border border-white/10 rounded-2xl overflow-hidden mb-8">
        <div className="px-6 py-6 flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl overflow-hidden bg-white/10 flex items-center justify-center flex-shrink-0">
            {kennel.logo_url ? <img src={kennel.logo_url} alt="" className="w-full h-full object-cover" /> : <span className="text-3xl font-bold text-white/20">{kennel.name[0]}</span>}
          </div>
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 mt-3">
              {kennel.foundation_date && <span className="flex items-center gap-1.5 text-xs text-white/40"><Calendar className="w-3.5 h-3.5" /> Fundado en {new Date(kennel.foundation_date).getFullYear()}</span>}
              {kennel.website && <a href={kennel.website} target="_blank" rel="noopener" className="flex items-center gap-1.5 text-xs text-white/40 hover:text-[#D74709] transition"><Globe className="w-3.5 h-3.5" /> Web</a>}
              {kennel.social_instagram && <a href={kennel.social_instagram} target="_blank" rel="noopener" className="flex items-center gap-1.5 text-xs text-white/40 hover:text-pink-400 transition"><ExternalLink className="w-3.5 h-3.5" /> Instagram</a>}
      ...
      {forSale.length > 0 && (
        <section className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-white/30 bg-white/5 rounded-full px-2 py-0.5">{forSale.length}</span>
          </div>
      ...
      <section>
        ...
        {criados.length === 0 && forSale.length === 0 && reproductores.length === 0 ? (
          <div className="text-center py-12 bg-white/5 border border-white/10 rounded-xl text-white/30">No hay perros visibles en este criadero</div>
      ...
      <div className="p-2.5">
        ...
        <div className="flex items-center justify-between mt-1">
          {dog.sale_price ? (
            ...
            <p className="text-xs text-white/30">Consultar precio</p>
          )}
          {dog.sale_location && <p className="text-[10px] text-white/30 truncate ml-2">{dog.sale_location}</p>}
        </div>
      </div>
    </Link>
  )
