  ...
  if (!kennelId) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20">
        ...
        <p className="text-white/60 mb-2">Necesitas tener un criadero para gestionar API keys.</p>
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-white/40">{kennelName}</p>
        </div>
      </div>
      ...
      {newKey && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 mb-6">
          ...
          <div className="flex items-center gap-2 bg-gray-950 border border-emerald-500/20 rounded-lg p-3">
          ...
          <button onClick={() => setNewKey(null)} className="text-xs text-white/50 hover:text-white/80 mt-3">
            Cerrar
          </button>
        </div>
      )}
      ...
      {showCreate ? (
        <form onSubmit={handleCreate} className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6">
          ...
          <input
            ...
            required autoFocus
            className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-[#D74709] focus:outline-none transition mb-3"
          />
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => { setShowCreate(false); setNewKeyName(''); setError('') }}
              className="text-sm text-white/40 hover:text-white/70 px-3 py-2">
              Cancelar
            </button>
          </div>
        </form>
      ...
      <div className="space-y-2 mb-6">
        <h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider">Activas ({activeKeys.length})</h3>
        {activeKeys.length === 0 ? (
          <p className="text-xs text-white/30 py-4 text-center bg-white/[0.02] rounded-lg">Sin keys activas</p>
        ) : (
          activeKeys.map(k => (
            <div key={k.id} className="bg-white/5 border border-white/10 rounded-lg p-3 flex items-center gap-3">
      ...
      {revokedKeys.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider">Revocadas ({revokedKeys.length})</h3>
          {revokedKeys.map(k => (
            <div key={k.id} className="bg-white/[0.02] border border-white/5 rounded-lg p-3 flex items-center gap-3 opacity-50">
              <Key className="w-4 h-4 text-white/20 flex-shrink-0" />
              <div className="flex-1 min-w-0">
