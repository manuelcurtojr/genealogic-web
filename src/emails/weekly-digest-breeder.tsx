...
export default function WeeklyDigestBreederEmail({
  recipientName, kennelName, weekLabel, stats, topMessages = [],
  ...
  return (
    <EmailLayout preview={`Tu semana en ${kennelName}: ${stats.newReservations} reservas, ${stats.pendingMessages} mensajes`}>
      <Eyebrow>Resumen semanal</Eyebrow>
      <H1>
        {name ? `${name}, ` : ''}
        esto pasó esta semana en {kennelName}.
      </H1>
      ...
      <table cellPadding={0} cellSpacing={0} style={{ borderCollapse: 'collapse', width: '100%', margin: '20px 0' }}>
        <tr>
          <td style={{ width: '50%', verticalAlign: 'top', paddingRight: '6px', paddingBottom: '12px' }}>
            <StatBlock value={stats.newReservations} label="Reservas nuevas" />
          </td>
          <td style={{ width: '50%', verticalAlign: 'top', paddingLeft: '6px', paddingBottom: '12px' }}>
            <StatBlock value={stats.pendingMessages} label="Mensajes pendientes" />
          </td>
        </tr>
        <tr>
          <td style={{ width: '50%', verticalAlign: 'top', paddingRight: '6px' }}>
            <StatBlock value={stats.upcomingLitters} label="Camadas activas" />
          </td>
          <td style={{ width: '50%', verticalAlign: 'top', paddingLeft: '6px' }}>
            <StatBlock value={stats.profileViews} label="Visitas a tu web" />
          </td>
        </tr>
      </table>
      ...
      {total === 0 && (
        <P style={{ color: COLORS.muted, fontSize: 13, marginTop: '20px' }}>
          Semana tranquila. Aprovecha para subir fotos nuevas o actualizar la web —
          los kennels con perfil actualizado reciben 3× más visitas.
        </P>
      )}
